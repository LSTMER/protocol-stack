...
uint16_t transport_checksum(uint8_t protocol, buf_t *buf, uint8_t *src_ip, uint8_t *dst_ip) {
    ...
    ip_hdr_t *ip_temp = (ip_hdr_t*)malloc(sizeof(ip_hdr_t));
    memcpy(ip_temp, ip_hdr, sizeof(ip_hdr_t));
    ...
    if(buf->len%2==1){
        ...
        buf_add_header(buf, sizeof(ip_hdr_t));
        ip_hdr_t *ip_hdr1 = (ip_hdr_t *)buf->data;
        memcpy(ip_hdr1, ip_temp, sizeof(ip_hdr_t));
        buf_remove_header(buf, sizeof(ip_hdr_t));
        free(ip_temp);
